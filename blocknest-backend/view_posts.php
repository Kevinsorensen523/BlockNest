...
if (isset($_POST['notification'])) {
    ...
    $query = "SELECT * FROM post WHERE post_id = ? ORDER BY post_id DESC ,date_posted DESC, (likes * 0.7 + comment_no * 0.3) DESC";
    $stmt = $db->prepare($query);
    $stmt->bind_param('i', $notification);
    $stmt->execute();
    ...
    $response['success'] = 1;
    $response['message'] = "Posts containing '{$notification}' retrieved successfully.";
} else {
    $response['success'] = 0;
    $response['message'] = "No notification provided.";
}
