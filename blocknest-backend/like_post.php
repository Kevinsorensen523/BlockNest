    ...
    if($type == "check") {
        ...
        if($checkStmt->num_rows == 0) {
            ...
            if ($stmt->execute()) {
                ...
                $interactionQuery = "INSERT INTO user_interactions(post_id, user_id, target_user_id, action_type, isOpen) VALUES (?, ?, ?, 'like', FALSE)";
        ...
        if ($stmt->execute()) {
            ...
            $interactionQuery = "DELETE FROM user_interactions WHERE post_id = ? AND user_id = ? AND action_type = 'like'";
            $interactionStmt = $db->prepare($interactionQuery);
            $interactionStmt->bind_param('ii', $post_id, $user_id);
            $interactionStmt->execute();
        }
    }
